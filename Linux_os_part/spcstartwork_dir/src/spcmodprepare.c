...
int _getCheckFileName(char *dataFileName, uint32_t dataFileNameLen, char *checkFileNameBuf, uint32_t checkFileNameBufLen)
{
	//check
...
{
	//check input
	if((NULL == sig) || (0 == sigLen) || (NULL == md5) || (0 == md5Len))
	{
		printf("_checkSig():");
		printf("Got invalid inputs.\n");
		return -1;
	...
	//checksig
	int ret = -1;
	ret = spc_verify_sig_with_md5(sig, sigLen, md5, md5Len);
	return ret;	
}
...
char *_findDataFile(char *dirPath, uint32_t *retNameLen)
{
	//check inputs
...
int prepare_processDataFile()
{
	...
	//check sig
	//free sig
	ret = _checkSig(sig, sigLen, md5, 32);
	if(ret == -1)
	{
		printf("prepare_processDataFile():sig in checksum file invalid.\n");
		free(retFileName);
		free(sig);
		return -1;
